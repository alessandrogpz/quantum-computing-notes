...
# 03_Protocols -- BB84
# =============================================================================
# Both of these come straight out of 03_Protocols/bb84.py, so the picture in the
# note is drawn from the object that runs. The barriers separate the three parties:
# they are in the circuit to stop the transpiler folding Alice's basis rotation
# into Bob's, and they happen to be exactly where a reader wants a divider too.

@figure("circuit_bb84_round", width=560)
def _bb84_round():
    """A clean round: Alice sends |->, Bob happens to pick the same basis."""
    sys.path.insert(0, str(ROOT / "03_Protocols"))
    from bb84 import round_circuit  # noqa: E402
    return round_circuit((1, "X", None, "X"))


@figure("circuit_bb84_eve", width=680)
def _bb84_eve():
    """The same round with Eve guessing Z, the case that costs Bob his certainty."""
    sys.path.insert(0, str(ROOT / "03_Protocols"))
    from bb84 import round_circuit  # noqa: E402
    return round_circuit((1, "X", "Z", "X"))
...
@figure("circuit_e91_round", width=620)
def _e91_round():
    """A clean round at settings A1, B1 -- one of the four CHSH terms."""
    sys.path.insert(0, str(ROOT / "03_Protocols"))
    from e91 import round_circuit  # noqa: E402
    return round_circuit((0, 0, None))


@figure("circuit_e91_eve", width=740)
def _e91_eve():
    """The same round with Eve entangled to Alice's qubit, which is what kills S."""
    sys.path.insert(0, str(ROOT / "03_Protocols"))
    from e91 import round_circuit  # noqa: E402
    return round_circuit((0, 0, "Z"))
...
def write_manifest(built: list[tuple[str, int]], args) -> None:
    ...
    # Explicit UTF-8: the manifest contains em dashes, and write_text() would
    # otherwise use the machine's locale encoding and quietly emit cp1252 on
    # Windows -- which is not valid UTF-8, so Obsidian and GitHub would both
    # mis-render a file that looked fine to the machine that wrote it.
    (OUT / "00_Figures.md").write_text(encoding="utf-8", data=f"""# Figures
